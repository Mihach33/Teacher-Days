    ...
    /// </summary>
    [ExecuteInEditMode]
    public class CodeMarz : MonoBehaviour, IMaterialModifier {
        ...
        /// </summary>
        public static readonly int MaxPolygonVertices = 200;
        ...
        /// </summary>
        public class ShapeDrawOrderComparer : IComparer<CodeMarz> {
            public int Compare(CodeMarz s1, CodeMarz s2) {
                ...
                if (s1.spriteRenderer) {
                    ...
                    foreach (CodeMarz s in s1.image.canvas.GetComponentsInChildren<CodeMarz>()) {
                        if (s == s1)
                            break;
                        order1 ++;
                    }
                ...
                if (s2.spriteRenderer) {
                    ...
                    foreach (CodeMarz s in s2.image.canvas.GetComponentsInChildren<CodeMarz>()) {
                        if (s == s2)
                            break;
                        order2 ++;
                    }
            ...
            public PolygonPreset polygonPreset {
                ...
                set {
                    if (value != PolygonPreset.Custom)
                        _polyVertices = CodeMarz.GetVerticesForPreset(value);
        ...
        bool UIShapeIsMasked(CodeMarz shape, bool includeSelf) {
        ...
        bool UIShapeIsRectMasked(CodeMarz shape, bool includeSelf) {
        ...
        public Bounds GetShapeBounds() {
            ...
            foreach (CodeMarz shape in GetComponentsInChildren<CodeMarz>()) {
                if (!shape.enabled)
                    continue;
        ...
        /// </summary>
        public List<CodeMarz> GetShapesInDrawOrder() {
            List<CodeMarz> shapes = new List<CodeMarz>();
            shapes.AddRange(GetComponentsInChildren<CodeMarz>());
        ...
        public List<Object> GetUndoObjects() {
            ...
            foreach (CodeMarz s in GetComponentsInChildren<CodeMarz>())
